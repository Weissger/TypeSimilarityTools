...
class TypeSimilarityTools(object):
    ...
    def __init__(self, server, user, password, n_processes, log_level, similarity_store='data/similarities.db'):
        ...
        if similarity_store:
            self.__similarity_store = SQLiteStore(similarity_store)

    def get_type_similarity(self, type_a, type_b, force_calc=False):
        if not force_calc and self.__similarity_store.has_similarity(type_a, type_b):
            return self.__similarity_store.get_similarity(type_a, type_b)
